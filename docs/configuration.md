...
- `SESSION_COOKIE_SECURE`: Optional override for the session cookie `Secure` flag. Leave it unset to auto-detect from the current request protocol, set it to `false` for plain HTTP LAN hosting, or `true` to force HTTPS-only cookies.
